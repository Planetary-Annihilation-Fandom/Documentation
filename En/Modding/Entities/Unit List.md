...
The game loads entities using a list from the file **"unit_list.json"**. This file is located in **"pa/units/unit_list.json"**. Any mod that adds buildings and units must have this file.

## How it works

The json file contains only one field - **"units"** storing the list. The list values are the paths to the Main json file of each entity.

The game engine works in such a way that the list of entities should contain not only new entities, but also the entities of the base game. When loading mods, the engine uses a list of mod entities with the highest priority. Therefore, if you do not add basic entities, they will not appear in the game, there will only be mod entities.

## Several modifications

Several modifications with their own entities can work simultaneously using the **"merge"** functionality. The game engine takes lists of entities from all modifications and combines them into one. Merge can be enabled in the game in the window **"Community Mods"**.
...
"/pa/units/commanders/imperial_fusion/imperial_fusion.json",
...
"/pa/units/commanders/quad_pumpkin/quad_pumpkin.json",
...
"/pa/units/commanders/raptor_beast_king/raptor_beast_king.json",
